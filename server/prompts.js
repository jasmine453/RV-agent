...
3. **日期格式要求**：
   ...
   - 债权人会议报告日期格式：XXXX 年 X 月 X 日 或 XXXX 年 XX 月 XX 日（根据模板格式要求，使用数字年份）
   - 庭外重组协议日期格式：根据模板格式要求
...
❌ 错误的表格格式（禁止）：
序号债权编号债权人名称债权金额1PT002某公司100万元2PT003某人50万元`,
        user: async (documentText, dates) => {
            await initializeTemplates();
            const template = templateCache.meetingReport || '【模板加载失败】';
...
1. ✅ 必须替换所有占位符：
   - XXX、XXXX、XX、____（下划线）等占位符必须用实际信息替换
   - 如果文档中没有相关信息，使用"[待补充]"而不是保留XXX
   - 绝对不能在最终输出中保留任何XXX、XX占位符！
   
2. 📋 替换示例：
   - "XXX律师事务所" → 从文档提取实际名称，或使用"[待补充]律师事务所"
   - "XXXX年XX月XX日" → 使用当前日期或"[待补充]年[待补充]月[待补充]日"
   - "债权人XXX" → 从文档提取，或使用"债权人[待补充]"
   
3. ❌ 禁止改动：模板的任何文字、格式、段落结构
4. 🚫 不要添加：解释、说明或任何额外内容
...
【⚠️ 表格格式特别强调】
1. 表格必须保持清晰的行列结构
2. 每一行数据必须独立成行（用换行符分隔）
3. 列与列之间用适当空格分隔
4. 表头和数据行都要清晰可辨

示例：
第三顺序普通破产债权
序号  债权编号  普通债权待确认  诉讼中未提供证据材料
1     PT002    待确认         对补充材料

而不是：
第三顺序普通破产债权1PT002普通债权待确认诉讼中未提供证据材料对补充材料
...
【开始输出】直接输出债权人会议报告正文（保持所有换行和表格格式）：`;
        }
    },
...
function getTimeoutConfig(analysisType) {
    ...
    const longFormTypes = [
        ...
        'outside-agreement',
        'meeting-report',
        // 以下分析类报告通常输出字数很多，也提升到长超时配置，避免 2 分钟即被中断
        'enterprise-value',
        'risk-indicators',
        'restructure-feasibility'
    ];
    return {
        timeout: longFormTypes.includes(analysisType) ? 600000 : 120000, // 10分钟 或 2分钟
        maxTokens: longFormTypes.includes(analysisType) ? 8000 : 4000, // DeepSeek最大支持8192
        temperature: longFormTypes.includes(analysisType) ? 0.3 : 0.1 // 提高temperature使AI更灵活地添加换行
    };
}
