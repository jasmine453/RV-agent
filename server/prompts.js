...
function getTimeoutConfig(analysisType) {
    ...
    // 【终极修复】大幅降低maxTokens避免Gunzip过早关闭
    // Render + DeepSeek 实测稳定值（2024-12-03）
    // 原因：输入+输出总token过大 → 响应体过大 → Gunzip解压失败
    if (longFormTypes.includes(analysisType)) {
        return {
            timeout: 600000,    // 10分钟
            maxTokens: 4000,    // 大幅降低（从6000→4000），避免响应体过大
            temperature: 0.3
        };
    } else if (mediumFormTypes.includes(analysisType)) {
        return {
            timeout: 300000,    // 5分钟
            maxTokens: 3500,    // 降低（从5000→3500）
            temperature: 0.2
        };
    } else {
        return {
            timeout: 120000,    // 2分钟
            maxTokens: 3000,    // 降低（从4000→3000）
            temperature: 0.1
        };
    }
}
...
/**
 * 【修复 Premature close】智能截断文本，避免超过API限制
 * DeepSeek API 实测限制：连接会在处理过大内容时提前关闭
 * 
 * 经验值（经过多次测试优化）：
 * - 庭外重组协议/预重整方案：6,000字符（避免Premature close）
 * - 其他分析：8,000字符
 * - 警戒值：10,000字符
 * - 危险值：12,000+字符（极易失败）
 */
function truncateDocumentText(text, maxChars = 8000) {
