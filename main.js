...
async function generateClaimsMeetingReport() {
    ...
    try {
        ...
        if (result.success) {
            ...
            // 高亮待补充内容（启用可编辑模式）
            const highlightedReport = highlightPlaceholders(cleanedResult, false, true);
            ...
            textDisplay.innerHTML = `
                <div style="padding: 1.5rem; text-align: left;">
                    ...
                    <div id="editableContent" contenteditable="true" style="background: var(--bg-white); padding: 2rem; border-radius: 8px; border: 1px solid var(--border-color); max-height: 600px; overflow-y: auto; text-align: left; cursor: text; white-space: pre-wrap; font-family: 'Microsoft YaHei', 'SimSun', Arial, sans-serif; line-height: 1.8; font-size: 14px; color: var(--text-dark); text-align: justify; word-wrap: break-word;" onfocus="this.style.borderColor='#4CAF50'" onblur="this.style.borderColor='var(--border-color)'">${highlightedReport}</div>
                    ${placeholderSummary}
                    <div style="margin-top: 1rem; padding: 0.75rem; background: #f0f9ff; border-radius: 6px; font-size: 0.85rem; color: #0369a1; text-align: left;">
                        <strong>会议报告完成</strong> - 基于会议文档智能生成 | <span style="color: #4CAF50;">可在线编辑</span>
                    </div>
                    <div style="margin-top: 1.5rem; display: flex; gap: 1rem; flex-wrap: wrap; justify-content: flex-start;">
                        <button onclick="copyEditableContent()" style="
                            padding: 0.75rem 1.5rem;
                            ...
                            font-weight: 600;
                        ">复制内容</button>
                        <button onclick="applyEditsAndDownload('report')" style="
                            padding: 0.75rem 1.5rem;
                            background: transparent;
                            color: #000000;
                            border: 2px solid #000000;
                        ...
                        <button onclick="downloadDocument('report')" style="
                            ...
                            font-weight: 600;
                        ">下载报告</button>
                        <button onclick="previewDocument('report')" style="
                            padding: 0.75rem 1.5rem;
                            background: transparent;
                            color: #000000;
                            border: 2px solid #000000;
...
async function generateBothCharts() {
    showLoadingMessage('正在同时生成企业价值图表和风险雷达图...');
...
async function generateChartImage(chartType) {
    try {
        ...
        // 准备请求数据 - 包含分析结果
        const requestData = {
            analysisData: generatedContent[analysisKey],
            chartType: chartType
        };
        
        if (chartType === 'feasibility') {
            // 尝试从分析结果中提取评分
            const scoreMatch = generatedContent[analysisKey].match(/(\d+)分/);
            requestData.feasibilityScore = scoreMatch ? parseInt(scoreMatch[1]) : 75;
        }
        
        console.log('📤 发送请求数据:', {
            endpoint: `/generate-chart/${chartType}`,
            dataLength: requestData.analysisData?.length || 0,
        ...
        const iconMap = {
            ...
            'feasibility': `<svg width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
