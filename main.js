    ...
    try {
        ...
        textDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="agreementProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">正在初始化任务...</p>
            </div>
        `;
        
        // 调用AI API生成庭外重组协议
        const initialResponse = await callAPI('/analyze', {
            files: uploadedFiles,
        ...
        if (initialResponse.taskId) {
            console.log('📋 收到庭外重组协议任务ID:', initialResponse.taskId);
            const progressMessageElement = document.getElementById('agreementProgressMessage');
            
            result = await pollTaskStatus(initialResponse.taskId, (taskData) => {
                if (progressMessageElement) {
                    progressMessageElement.textContent = `${taskData.message} (${taskData.progress}%)`;
                }
                if (loadingToast && loadingToast.querySelector('.toast-message')) {
                    loadingToast.querySelector('.toast-message').textContent = 
                        `${taskData.message} (${taskData.progress}%)`;
                }
            });
        } else {
    ...
    try {
        ...
        textDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="planProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">正在初始化任务...</p>
            </div>
        `;
        ...
        // 检查是否返回了taskId（异步处理）
        if (initialResponse.taskId) {
            console.log('📋 收到预重整方案任务ID:', initialResponse.taskId);
            const progressMessageElement = document.getElementById('planProgressMessage');
...
async function analyzeEnterpriseValue() {
    ...
    let progressMessage = '正在分析企业价值，请稍候...';
    const loadingToast = showLoadingMessage(progressMessage);
    ...
    try {
        ...
        analysisDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="analysisProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">正在初始化分析任务...</p>
            </div>
        `;
        
        // 调用AI API进行企业价值分析（返回taskId）
        const initialResponse = await callAPI('/analyze', {
            files: uploadedFiles,
        ...
        // 检查是否返回了taskId（异步处理）
        if (initialResponse.taskId) {
            console.log('📋 收到任务ID:', initialResponse.taskId);
            
            // 更新进度消息
            const progressMessageElement = document.getElementById('analysisProgressMessage');
            if (progressMessageElement) {
                progressMessageElement.textContent = '任务已创建，正在后台处理...';
            }
            
            // 轮询任务状态
            const result = await pollTaskStatus(initialResponse.taskId, (taskData) => {
                // 更新进度显示
                if (progressMessageElement) {
                    progressMessageElement.textContent = `${taskData.message} (${taskData.progress}%)`;
                }
            ...
            if (initialResponse.success) {
                const cleanedResult = cleanMarkdownSymbols(initialResponse.result);
                generatedContent.analysis = cleanedResult;
            ...
            `;
                ...
                throw new Error(initialResponse.message || '分析失败');
            }
        }
    } catch (error) {
        hideLoadingMessage();
    ...
    try {
        ...
        analysisDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="valueProgressMessage" style="color: var(--text-light); font-size: 0.9rem; margin-top: 1rem;">企业价值分析：正在初始化...</p>
                <p id="riskProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">风险指标分析：正在初始化...</p>
            </div>
        `;
        
        // 并行调用两个分析API
        const [valueInitialResponse, riskInitialResponse] = await Promise.all([
            callAPI('/analyze', {
                files: uploadedFiles,
                analysisType: 'enterprise-value'
            }),
        ...
        let valueResult, riskResult;
        
        // 处理企业价值分析
        if (valueInitialResponse.taskId) {
            console.log('📋 企业价值分析任务ID:', valueInitialResponse.taskId);
            const valueProgressElement = document.getElementById('valueProgressMessage');
            valueResult = await pollTaskStatus(valueInitialResponse.taskId, (taskData) => {
                if (valueProgressElement) {
                    valueProgressElement.textContent = `企业价值分析：${taskData.message} (${taskData.progress}%)`;
                }
            });
        } else {
            valueResult = valueInitialResponse;
        }
        
        // 处理风险指标分析
        if (riskInitialResponse.taskId) {
            console.log('📋 风险指标分析任务ID:', riskInitialResponse.taskId);
            const riskProgressElement = document.getElementById('riskProgressMessage');
            riskResult = await pollTaskStatus(riskInitialResponse.taskId, (taskData) => {
                if (riskProgressElement) {
                    riskProgressElement.textContent = `风险指标分析：${taskData.message} (${taskData.progress}%)`;
                }
            });
        } else {
            riskResult = riskInitialResponse;
        }
    ...
    try {
        ...
        analysisDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="riskExtractProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">正在初始化任务...</p>
            </div>
        `;
        ...
        // 检查是否返回了taskId（异步处理）
        if (initialResponse.taskId) {
            console.log('📋 收到风险指标任务ID:', initialResponse.taskId);
            const progressMessageElement = document.getElementById('riskExtractProgressMessage');
...
async function generateRestructureFeasibility() {
    ...
    const analysisDisplay = document.getElementById('analysisDisplay');
    if (!analysisDisplay) {
        console.error('找不到 analysisDisplay 元素');
        showErrorMessage('页面元素加载失败，请刷新页面重试');
        return;
    }
    ...
    try {
        ...
        analysisDisplay.innerHTML = `
            <div style="padding: 2rem; text-align: center;">
                ...
                <p id="feasibilityProgressMessage" style="color: var(--text-light); font-size: 0.9rem;">正在初始化任务...</p>
            </div>
        `;
        ...
        // 检查是否返回了taskId（异步处理）
        if (initialResponse.taskId) {
            console.log('📋 收到可行性分析任务ID:', initialResponse.taskId);
            const progressMessageElement = document.getElementById('feasibilityProgressMessage');
...
async function pollTaskStatus(taskId, onProgress = null, maxAttempts = 150, interval = 8000) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            const response = await fetch(`${API_CONFIG.baseURL}/task/${taskId}`, {
                signal: controller.signal
            });
            ...
            console.log(`任务状态 [${attempt}/${maxAttempts}]:`, taskData.status, `- 进度: ${taskData.progress}%`);
            ...
            if (taskData.status === 'completed') {
                ...
                return {
                    ...
                    processingTime: taskData.processingTime
                };
            }
            
            if (taskData.status === 'failed') {
                console.error(`❌ 任务失败: ${taskId}`, taskData.error);
                const error = new Error(taskData.message || '任务处理失败');
                error.errorType = taskData.errorType || 'task_failed';
                error.errorDetails = taskData.errorDetails || taskData.error || '未知错误';
                error.fullError = taskData.fullError || taskData.error || '';
                throw error;
            }
            ...
            consecutiveErrors++;
            console.error(`⚠️ 查询任务状态时出错 [${attempt}/${maxAttempts}] (连续错误: ${consecutiveErrors}/${maxConsecutiveErrors}):`, error.message);
            
            // 如果连续错误次数过多，才真正放弃
            if (consecutiveErrors >= maxConsecutiveErrors) {
                console.error(`❌ 连续${maxConsecutiveErrors}次查询失败，任务可能已失败`);
                throw error;
            }
            
            // 否则等待后重试（使用指数退避）
            const waitTime = Math.min(interval * Math.pow(1.5, consecutiveErrors - 1), 30000);
    ...
    // 超过最大轮询次数
    const timeoutError = new Error('任务处理超时');
    timeoutError.errorType = 'timeout';
    timeoutError.errorDetails = `任务在${maxAttempts * interval / 60000}分钟内未完成。\n\n可能的原因：\n1. 文档内容过多，AI分析时间较长\n2. AI服务响应缓慢\n3. Render免费版资源限制\n\n建议：\n- 稍后在浏览器控制台运行以下代码查看结果：\n  fetch('/api/task/${taskId}').then(r => r.json()).then(console.log)\n- 尝试减少上传文档的数量或大小（推荐2-3个核心文档）\n- 等待5-10分钟后刷新页面重试`;
    throw timeoutError;
}
