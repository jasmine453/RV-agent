...
// 【Render优化】任务持久化到文件系统
const TASK_STORAGE_DIR = path.join(__dirname, 'uploads', '.tasks');
if (!fs.existsSync(TASK_STORAGE_DIR)) {
    fs.mkdirSync(TASK_STORAGE_DIR, { recursive: true });
}

// 保存任务到文件
function saveTaskToFile(taskId, taskData) {
    try {
        const taskFile = path.join(TASK_STORAGE_DIR, `${taskId}.json`);
        fs.writeFileSync(taskFile, JSON.stringify(taskData, null, 2));
    } catch (error) {
        console.error(`保存任务文件失败 (${taskId}):`, error.message);
    }
}
...
function loadTaskFromFile(taskId) {
    try {
        ...
        console.error(`加载任务文件失败 (${taskId}):`, error.message);
    }
    return null;
}
...
function setTask(taskId, taskData) {
    ...
    saveTaskToFile(taskId, taskData);
}

// 增强的任务获取（先从内存获取，失败则从文件获取）
        ...
        if (useAsync) {
            ...
            (async () => {
                try {
                    ...
                    // 【终极修复】大幅降低截断长度，避免Gunzip过早关闭错误
                    // Render + DeepSeek 组合实测稳定值（2024-12-03）
                    const maxCharsMap = {
                        'outside-agreement': 4000,        // 庭外重组协议（从6000降低）
                        'pre-restructure-plan': 4000,     // 预重整方案（从6000降低）
                        'restructure-feasibility': 4500,  // 重组可行性报告（从7000降低）
                        'enterprise-value': 5000,         // 企业价值评估（从8000降低）
                        'value-risk': 5000,               // 价值与风险（从8000降低）
                        'risk-indicators': 5000           // 风险指标（从8000降低）
                    };
                    const maxChars = maxCharsMap[analysisType] || 5000;
...
app.get('/api/task/:taskId', (req, res) => {
    ...
    const task = getTask(taskId);  // 使用增强的getTask，支持从文件恢复
...
setInterval(() => {
    ...
    // 清理孤立的任务文件（文件存在但内存中没有）
    try {
        const taskFiles = fs.readdirSync(TASK_STORAGE_DIR);
        for (const file of taskFiles) {
            if (file.endsWith('.json')) {
                const taskId = file.replace('.json', '');
                const taskFile = path.join(TASK_STORAGE_DIR, file);
        ...
        console.error('清理任务文件时出错:', error.message);
    }
