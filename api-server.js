        ...
        if (useAsync) {
            ...
            (async () => {
                try {
                    ...
                    // 更新任务状态
                    const task30 = taskStorage.get(taskId);
                    if (!task30) {
                    ...
                    if (analysisResult.success) {
                        ...
                        const finalTask = {
                            ...
                            createdAt: taskStorage.get(taskId)?.createdAt || new Date().toISOString()
                        };
                        
                        taskStorage.set(taskId, finalTask);
                        ...
                        console.log(`错误详情: ${analysisResult.errorDetails}`);
                        console.log(`${'='.repeat(50)}\n`);
                        ...
                        taskStorage.set(taskId, failedTask);
                        console.log(`❌ 任务 ${taskId} 状态已更新为 failed`);
                    }
                } catch (error) {
                    const elapsedTime = ((Date.now() - startTime) / 1000).toFixed(2);
